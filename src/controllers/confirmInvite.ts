...
export const confirmInvite = async (req :Request, res :Response):Promise<any> => {
    ...
    const messageBuffer = Buffer.from(
        JSON.stringify({ email, status: "CONFIRMED" })
    );

    await pubSubClient.topic(TOPIC_NAME).publishMessage({ data: messageBuffer });
    
    return res.json({ message: `Invite confirmed for ${email}` });
};